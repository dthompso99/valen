...
- The initial generation-0 AArch64 backend emits and directly encodes a deliberately restricted
  ...
  capacity, width-correct loads and stores, checked indexing with runtime status 70, append, reserve, and
  shrink-to-fit. Buffer growth uses allocate-copy-update until tracing reclamation is available. It is the
