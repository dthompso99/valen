...
The bootstrap is complete when the JavaScript compiler can build a Valen compiler, and that generated compiler can compile its own source again without using JavaScript at runtime.
...
The first checkpoint was a Valen tokenizer that read a source file and printed its token stream.
