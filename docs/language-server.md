...
This first implementation intentionally avoids editor-specific packaging. Issue #66 tracks the optional VS Code extension that can launch this server and associate it with `.ar` files. Future work may add completion, references, rename, semantic tokens, and finer-grained incremental analysis without changing the protocol boundary.
