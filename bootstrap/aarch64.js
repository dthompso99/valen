...
export class AArch64Backend {
    ...
    allocationRuntime() {
        return ['.globl valen_alloc', '.type valen_alloc, %function', 'valen_alloc:',
            '    cbnz x0, .Lalloc_size_ready', '    mov x0, #1', '.Lalloc_size_ready:',
            ...this.address('x9', 'valen_arena_enabled'), '    ldr w10, [x9, #0]', '    cbz w10, .Lalloc_direct',
            '    sub sp, sp, #64', '    str x19, [sp, #0]', '    str x20, [sp, #8]', '    str x21, [sp, #16]',
            '    str x22, [sp, #24]', '    str x30, [sp, #56]',
            '    add x19, x0, #31', '    lsr x19, x19, #4', '    lsl x19, x19, #4',
            ...this.address('x9', 'valen_arena_lock'), '.Lalloc_arena_lock:', '    ldaxr w10, [x9]',
            '    cbnz w10, .Lalloc_arena_lock', '    mov x11, #1', '    stlxr w12, w11, [x9]',
            '    cbnz w12, .Lalloc_arena_lock', '    mov x21, #0', ...this.address('x9', 'valen_arena_free_list'),
            '    ldr x22, [x9, #0]', '.Lalloc_arena_free_search:', '    cbz x22, .Lalloc_arena_bump',
            '    ldr x10, [x22, #8]', '    cmp x10, x19', '    b.cs .Lalloc_arena_free_found',
            '    mov x21, x22', '    ldr x22, [x22, #0]', '    b .Lalloc_arena_free_search',
            '.Lalloc_arena_free_found:', '    ldr x10, [x22, #0]', '    cbz x21, .Lalloc_arena_free_head',
            '    str x10, [x21, #0]', '    b .Lalloc_arena_free_ready', '.Lalloc_arena_free_head:',
            '    str x10, [x9, #0]', '.Lalloc_arena_free_ready:', '    mov x20, x22', '    ldr x19, [x22, #8]',
            '    b .Lalloc_arena_unlock', '.Lalloc_arena_bump:', ...this.address('x10', 'valen_arena_remaining'),
            '    ldr x11, [x10, #0]', '    cmp x11, x19', '    b.cc .Lalloc_arena_grow',
            ...this.address('x12', 'valen_arena_cursor'), '    ldr x20, [x12, #0]', '    add x13, x20, x19',
            '    str x13, [x12, #0]', '    sub x11, x11, x19', '    str x11, [x10, #0]',
            '    b .Lalloc_arena_unlock', '.Lalloc_arena_grow:', ...this.constant('x1', 268435456),
            ...this.constant('x14', 268435424), '    cmp x19, x14', '    b.ls .Lalloc_arena_chunk_ready',
            '    add x1, x19, #32',
            '.Lalloc_arena_chunk_ready:', '    mov x0, #0', '    mov x2, #3', '    mov x3, #34',
            '    mov x4, #-1', '    mov x5, #0', '    mov x8, #222', '    svc #0', '    mov x9, #-4095',
            '    cmp x0, x9', '    b.cs .Lallocation_error', '    mov x20, x0',
            ...this.address('x12', 'valen_arena_chunks'), '    ldr x13, [x12, #0]', '    str x13, [x20, #0]',
            '    add x13, x20, x1', '    str x13, [x20, #8]', '    str x20, [x12, #0]', '    add x20, x20, #32',
            ...this.address('x12', 'valen_arena_cursor'), '    add x13, x20, x19', '    str x13, [x12, #0]',
            ...this.address('x10', 'valen_arena_remaining'), '    sub x11, x1, x19', '    sub x11, x11, #32',
            '    str x11, [x10, #0]',
            '.Lalloc_arena_unlock:', '    str x19, [x20, #8]', ...this.address('x9', 'valen_arena_lock'),
            '    mov x10, #0', '    stlr w10, [x9]', '    add x0, x20, #16', '    ldr x19, [sp, #0]',
            '    ldr x20, [sp, #8]', '    ldr x21, [sp, #16]', '    ldr x22, [sp, #24]',
            '    ldr x30, [sp, #56]', '    add sp, sp, #64', '    ret', '.Lalloc_direct:',
            '    mov x1, x0', '    mov x0, #0', '    mov x2, #3', '    mov x3, #34', '    mov x4, #-1',
            '    mov x5, #0', '    mov x8, #222', '    svc #0', '    mov x9, #-4095',
            '    cmp x0, x9', '    b.cs .Lallocation_error', '    ret', '.size valen_alloc, .-valen_alloc', '',
            '.globl valen_free', '.type valen_free, %function', 'valen_free:',
            ...this.address('x9', 'valen_arena_enabled'), '    ldr w10, [x9, #0]', '    cbz w10, .Lfree_direct',
            ...this.address('x9', 'valen_arena_chunks'), '    ldr x9, [x9, #0]', '.Lfree_arena_chunk:',
            '    cbz x9, .Lfree_direct', '    add x10, x9, #32', '    cmp x0, x10', '    b.cc .Lfree_arena_next',
            '    ldr x10, [x9, #8]', '    cmp x0, x10', '    b.cc .Lfree_arena_found', '.Lfree_arena_next:',
            '    ldr x9, [x9, #0]', '    b .Lfree_arena_chunk', '.Lfree_arena_found:',
            '    sub x0, x0, #16', ...this.address('x9', 'valen_arena_lock'), '.Lfree_arena_lock:',
            '    ldaxr w10, [x9]', '    cbnz w10, .Lfree_arena_lock', '    mov x11, #1',
            '    stlxr w12, w11, [x9]', '    cbnz w12, .Lfree_arena_lock',
            ...this.address('x10', 'valen_arena_free_list'), '    ldr x11, [x10, #0]', '    str x11, [x0, #0]',
            '    str x0, [x10, #0]', '    mov x10, #0', '    stlr w10, [x9]', '    ret',
            '.Lfree_direct:', '    mov x8, #215', '    svc #0', '    ret', '.size valen_free, .-valen_free',
            ...
            '    str x0, [sp, #40]',
            ...(this.threading ? ['    bl valen_gc_heap_lock', '    ldr x0, [sp, #40]'] : []),
            ...
            ...this.address('x9', 'valen_arena_enabled'), '    ldr w10, [x9, #0]',
            '    cbnz w10, .Lgc_maybe_collect_done',
            ...
            '.Lgc_collect_unmap:', '    ldr x0, [sp, #16]', '    ldr x1, [x0, #8]', '    bl valen_free',
            '    b .Lgc_collect_sweep', '.Lgc_collect_done:',
            ...
            '    add sp, sp, #64', '.Lgc_collect_return:', '    ret', '.size valen_gc_collect, .-valen_gc_collect', '',
    ...
    arrayRuntime() {
        return ['.globl valen_array_new', '.type valen_array_new, %function', 'valen_array_new:',
            ...
            '    ldr x4, [x0, #16]', '    str x4, [sp, #32]', '    ldr x4, [x0, #8]', '    mul x4, x4, x2',
            '    cbnz x4, .Larray_resize_old_size', '    mov x4, #1', '.Larray_resize_old_size:',
            '    str x4, [sp, #40]', '    mov x0, x3', '    cbnz x0, .Larray_resize_allocate', '    mov x0, #1', '.Larray_resize_allocate:',
            ...
            '    ldr x2, [sp, #24]', '    str x1, [x0, #8]', '    str x2, [x0, #16]', '    ldr x0, [sp, #32]',
            '    ldr x1, [sp, #40]', '    bl valen_free', '    ldr x0, [sp, #0]', '    ldr x30, [sp, #56]',
    ...
    stringRuntime() {
        return ['.globl valen_string_new', '.type valen_string_new, %function', 'valen_string_new:',
            ...
            '    mov x1, #1', '.Lstring_finalize_size:', '    ldr x0, [x0, #0]', '    b valen_free',
            '.size valen_string_finalize, .-valen_string_finalize', '',
    ...
    gcData() {
        return ['.section .data', '.align 8', '.globl valen_gc_roots', 'valen_gc_roots:', '    .quad 0',
            ...
            '.globl valen_arena_cursor', 'valen_arena_cursor:', '    .quad 0',
            '.globl valen_arena_remaining', 'valen_arena_remaining:', '    .quad 0',
            '.globl valen_arena_chunks', 'valen_arena_chunks:', '    .quad 0',
            '.globl valen_arena_free_list', 'valen_arena_free_list:', '    .quad 0',
            '.globl valen_arena_lock', 'valen_arena_lock:', '    .quad 0',
            '.globl valen_arena_enabled', 'valen_arena_enabled:', '    .quad 0',
