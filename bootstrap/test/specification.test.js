...
test('normative specification has unique rule identifiers and valid local links', () => {
    ...
    for (const file of files) {
        ...
        for (const match of source.matchAll(/\*\*((?:LEX|MOD|NAM|TYP|OWN|OBJ|EVAL|VAL|NAT|TGT|COMP)-\d{3})\b/g)) {
    ...
    assert.ok(definitions.size >= 100, 'normative surface unexpectedly shrank');
    const conformance = fs.readFileSync(path.join(specificationDirectory, 'conformance.md'), 'utf8');
    for (const identifier of conformance.matchAll(/`((?:LEX|MOD|NAM|TYP|OWN|OBJ|EVAL|VAL|NAT|TGT|COMP)-\d{3})`/g)) {
